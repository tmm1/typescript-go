...
func matchFiles(path string, extensions []string, excludes []string, includes []string, caseSensitivity tspath.CaseSensitivity, currentDirectory string, depth *int, host vfs.FS, getFileSystemEntries func(path string, host vfs.FS) FileSystemEntries, realpath func(path string) string) []string {
	path = tspath.NormalizePath(path)
	currentDirectory = tspath.NormalizePath(currentDirectory)
...
func readDirectory(host vfs.FS, currentDir string, path string, extensions []string, excludes []string, includes []string, depth *int) []string {
	return matchFiles(path, extensions, excludes, includes, host.CaseSensitivity(), currentDir, depth, host, getAccessibleFileSystemEntries, host.Realpath)
}
