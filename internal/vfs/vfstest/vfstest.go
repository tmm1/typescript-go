...
func FromMapFS(m fstest.MapFS, caseSensitivity tspath.CaseSensitivity) vfs.FS {
	return vfs.FromIOFS(convertMapFS(m, caseSensitivity), caseSensitivity)
}

func convertMapFS(m fstest.MapFS, caseSensitivity tspath.CaseSensitivity) *mapFS {
	...
	for path, file := range newM {
		canonical := tspath.GetCanonicalFileName(path, caseSensitivity)
...
func (m *mapFS) Open(name string) (fs.File, error) {
	f, err := m.m.Open(tspath.GetCanonicalFileName(name, m.caseSensitivity))
	if err != nil {
		return nil, err
	}
...
func (m *mapFS) Realpath(name string) (string, error) {
	...
	canonical := tspath.GetCanonicalFileName(name, m.caseSensitivity)
